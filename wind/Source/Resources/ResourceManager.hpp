...
// conceptually struct Handle { uint32 index; uint32 generation }
...
using TVertexHandle = FHandle<uint32>;
using TIndexHandle = FHandle<uint32>;
using TModelHandle = FHandle<uint32>;
using TBufferHandle = FHandle<uint32>;
using TDynamicBufferHandle = FHandle<uint32>;
...
class FUResourceManager
{
  public:
	...
	WIND_NODISCARD auto CreateDefaultDepthImage(uint32 Width, uint32 Height) WIND_NOEXCEPT -> TWindResult<void>;
	...
	WIND_NODISCARD auto RecreateDefaultDepthImage(uint32 Width, uint32 Height) WIND_NOEXCEPT -> TWindResult<void>;
	...
	WIND_NODISCARD auto
	CreateDynamicBuffer(uint32 Size,
	                    vk::BufferUsageFlagBits Usage = vk::BufferUsageFlagBits::eUniformBuffer) WIND_NOEXCEPT
	    -> TWindResult<TDynamicBufferHandle>;

	WIND_NODISCARD auto CreateDynamicUniformBuffer(uint32 Size) WIND_NOEXCEPT -> TWindResult<TDynamicBufferHandle>;
