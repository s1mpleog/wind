...
{
	std::optional<uint32> AlbedoIndex;
	std::optional<uint32> NormalIndex;
	std::optional<uint32> MetallicRoughnessIndex;
	float Metallic{};
	float Roughness{};
...
WIND_NODISCARD WIND_INLINE auto DecodeWindAsset(std::span<const uint8> Buffer) WIND_NOEXCEPT -> TWindResult<FWindAsset>
{
	...
	std::array<const uint8, 8> WindMagic{
	    uint8{87}, uint8{73}, uint8{78}, uint8{68}, 0, 0, 0, 0,
	};
	...
	auto ReadU32 = [](std::span<const uint8> Buf, size_t &Cursor) -> uint32
	{
		uint32 Value{};
		std::memcpy(&Value, Buf.data() + Cursor, sizeof(uint32));
		Cursor += sizeof(uint32);
		return Value;
	};

	auto ReadU64 = [](std::span<const uint8> Buf, size_t &Cursor) -> uint64
	{
		uint64 Value{};
		std::memcpy(&Value, Buf.data() + Cursor, sizeof(uint64));
		Cursor += sizeof(uint64);
		return Value;
	};

	auto ReadF32 = [](std::span<const uint8> Buf, size_t &Cursor) -> float
	{
		float Value{};
	...
	// uint32 version     = read_u32(buffer, cursor);
	Cursor += sizeof(uint32);
	uint32 ChunkCount = ReadU32(Buffer, Cursor);

	if (ChunkCount < 2)
		WIND_ERR(WindError::internal());

	constexpr uint32 ChunkVert = 0x54524556; // "VERT"
	constexpr uint32 ChunkIndc = 0x43444E49; // "INDC"
	constexpr uint32 ChunkIuv = 0x5F565549;  // "IUV_"
	constexpr uint32 ChunkInor = 0x524F4E49; // "INOR"
	constexpr uint32 ChunkItan = 0x4E415449; // "ITAN"
	constexpr uint32 ChunkIsub = 0x42555349; // "ISUB"
	constexpr uint32 ChunkText = 0x54584554; // "TEXT"
	constexpr uint32 ChunkMate = 0x4554414D; // "MATE"
	constexpr uint32 ChunkEnd = 0x444E4549;  // "IEND"
	...
	for (uint32 I = 0; I < ChunkCount; I++)
	{
		...
		case ChunkItan:
		{
			uint64 Size = ReadU64(Buffer, Cursor);
		...
		case ChunkMate:
		{
			...
			Asset.Materials.emplace_back(FWindMaterial{
			    .AlbedoIndex =
			        AlbedoIndex == UINT32_MAX ? std::optional<uint32>{std::nullopt} : std::optional{AlbedoIndex},
			    .NormalIndex =
			        NormalIndex == UINT32_MAX ? std::optional<uint32>{std::nullopt} : std::optional{NormalIndex},
			    .MetallicRoughnessIndex = MetallicRoughnessIndex == UINT32_MAX ? std::optional<uint32>{std::nullopt}
...
WIND_NODISCARD WIND_INLINE auto ReadFile(std::string_view Name) WIND_NOEXCEPT -> TWindResult<std::vector<uint8>>
{
