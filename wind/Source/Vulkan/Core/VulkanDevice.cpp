                                        ...
                                        const vk::raii::SurfaceKHR &Surface) -> FPhysicalDeviceCandidate
{
	...
	auto FindQueue = [&](vk::QueueFlagBits Flag) -> std::optional<unsigned int>
	{
		...
		return It != QueueInfos.end() ? std::optional{static_cast<uint32>(std::distance(QueueInfos.begin(), It))}
		                              : std::nullopt;
	};

	auto FindPresentQueue = [&](vk::SurfaceKHR Surface) -> std::optional<uint32>
	{
		for (uint32 I = 0; I < QueueInfos.size(); I++)
		{
	...
	FPhysicalDeviceCandidate Candidate{
	    ...
	    .TransferQueueIndex = TransferQueue ? std::optional<uint32>{TransferQueue} : std::nullopt,
	};
...
WIND_NODISCARD static auto CreateCommandPool(const vk::raii::Device &Device, uint32 QueueIndex) WIND_NOEXCEPT
...
auto FVulkanPhysicalDeviceFeatures::Query(vk::PhysicalDevice PhysicalDevice, uint32 APIVersion) WIND_NOEXCEPT -> void
{
...
WIND_NODISCARD TWindResult<void> FVulkanDevice::CreateDevice() WIND_NOEXCEPT
{
