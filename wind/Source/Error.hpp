...
struct WindError
{
  ...
  public:
	...
	WIND_NODISCARD static auto sdl(ErrorCode code, std::source_location loc = std::source_location::current())
	    -> WindError
	{
	...
	WIND_NODISCARD auto to_string() const -> std::string
	{
...
#ifndef NDEBUG
		{
			...
			// ── source location ────────────────────────────────────────
			out += std::format("\n  {}Location{}  {}{}:{}{} in {}{}{}\n", CYAN, RESET, GRAY, location.file_name(),
			                   location.line(), RESET, DIM, location.function_name(), RESET);
			// ── stack trace ────────────────────────────────────────────
			out += std::format("\n  {}Stack Trace{}\n", CYAN, RESET);
  ...
  private:
	WindError(ErrorCode code, ErrorKind kind, vk::Result vk_result, std::string_view sdl_message,
	          std::source_location location)
	    : code{code}, kind{kind}, vk_result{vk_result}, sdl_message{sdl_message}, location{location}, trace{}
	{
